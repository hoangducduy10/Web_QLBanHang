...
<head>
    ...
    <title>DanhSachMauSac</title>
</head>
<body>
    <h1>Danh Sach Mau Sac</h1>
    ...
    <form action="/mau-sac/store" method="post">
        ...
        <div>
            ...
            <select name="trangThai">
                <option value="1">Con mau</option>
                <option value="0">Het mau</option>
            </select>
        ...
        <tbody>
            <c:forEach items="${lists.content}" var="sp">
                <tr>
                    ...
                    <td>${sp.ma}</td>
                    <td>${sp.ten}</td>
                    <td>${sp.trangThai==1?"Con mau":"Het mau"}</td>
                    <td>
                        <a href="/mau-sac/edit/${sp.id}">Update</a>
                        <a href="/mau-sac/delete/${sp.id}">Delete</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination">
            <li class="page-item ${lists.number == 0 ? 'disabled' : ''}">
                <a class="page-link" href="/mau-sac/index?page=${lists.number - 1}">Previous</a>
            </li>
                ...
                <c:choose>
                    <c:when test="${page == 0 || page == lists.totalPages - 1 || (page >= lists.number - 1 && page <= lists.number + 1)}">
                        <li class="page-item ${page == lists.number ? 'active' : ''}">
                            <a class="page-link" href="/mau-sac/index?page=${page}">${page + 1}</a>
                        </li>
                    </c:when>
                    <c:otherwise>
            ...
            <li class="page-item ${lists.number == lists.totalPages - 1 ? 'disabled' : ''}">
                <a class="page-link" href="/mau-sac/index?page=${lists.number + 1}">Next</a>
            </li>
        </ul>
    </nav>
